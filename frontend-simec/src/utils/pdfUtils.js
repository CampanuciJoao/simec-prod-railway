...
// VERSÃO FINAL CONSOLIDADA - SUPORTE A CHAMADOS NO HISTÓRICO, BI AVANÇADO E OS PROFISSIONAL
...
/**
PDF de Auditoria do Ativo (Histórico Individual do Equipamento).
*/
...
/**
PDF dos Relatórios Gerais (Inventário e Manutenções Realizadas).
*/
...
/**
PDF de Indicadores BI (Relatório Executivo de Performance).
*/
...
/**
 * 4. PDF de Ordem de Serviço Detalhada (Manutenção Individual)
 * Gera um documento completo com tempos reais, notas e dados do ativo.
 */
export const exportarOSManutencaoPDF = (m) => {
   ...
   // --- BLOCO 1: INFORMAÇÕES DO ATIVO ---
    ...
    // --- BLOCO 3: DESCRIÇÃO DO SERVIÇO ---
    doc.setFont(undefined, 'bold');
    doc.text("DESCRIÇÃO DO PROBLEMA / SERVIÇO:", 14, doc.lastAutoTable.finalY + 12);
    doc.setFont(undefined, 'normal');
    const desc = m.descricaoProblemaServico || 'Nenhuma descrição informada.';
    const splitDesc = doc.splitTextToSize(desc, 180);
    doc.text(splitDesc, 14, doc.lastAutoTable.finalY + 18);

    // --- BLOCO 4: HISTÓRICO DE NOTAS (O QUE ENVOLVEU) ---
    doc.setFillColor(241, 245, 249);
