...
function AbrirOsForm({ form, submitting, fieldErrors, statusOptions, onChange, onSubmit }) {
  ...
  // Apelido eh unico por unidade no SIMEC (regra do dominio): cada unidade
  // tem no maximo 1 equipamento com um dado apelido. Entao escolher um
  // apelido SEMPRE seleciona o equipamento direto. Apelidos parecidos em
  // outras unidades nao interferem porque ja filtramos por unidadeId.
  useEffect(() => {
    if (!selectedApelido) return;
    const alvo = equipamentosFiltrados[0];
    if (alvo && form.equipamentoId !== alvo.id) {
      onChange('equipamentoId', alvo.id);
    }
