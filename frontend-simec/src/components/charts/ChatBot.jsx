// Ficheiro: src/components/charts/ChatBot.jsx
...
function ChatBot() {
  ...
  const buscarDadosOSParaPDF = async (manutencaoId, token, apiBaseUrl) => {
    const response = await fetch(`${apiBaseUrl}/pdf-data/manutencao/${manutencaoId}`, {
      method: 'GET',
      headers: {
  ...
  const buscarDadosRelatorioParaPDF = async (ids, token, apiBaseUrl) => {
    const response = await fetch(`${apiBaseUrl}/pdf-data/relatorio`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      throw new Error(
        `Falha ao buscar dados do relatório para PDF. Status: ${response.status}`
      );
    }
  ...
  const buscarSeguroPorId = async (seguroId, token, apiBaseUrl) => {
    const response = await fetch(`${apiBaseUrl}/seguros/${seguroId}`, {
      method: 'GET',
      headers: {
        Authorization: token ? `Bearer ${token}` : '',
      },
  ...
  const executarAcaoAgente = async (resposta) => {
    ...
    try {
      if (resposta.acao === 'GERAR_PDF_OS') {
        ...
        const manutencao = await buscarDadosOSParaPDF(manutencaoId, token, apiBaseUrl);
      ...
      if (resposta.acao === 'GERAR_PDF_RELATORIO') {
        ...
        const resultado = await buscarDadosRelatorioParaPDF(ids, token, apiBaseUrl);
      ...
      if (resposta.acao === 'ABRIR_OS') {
        ...
        window.open(
          `/manutencoes/detalhes/${manutencaoId}`,
          '_blank',
          'noopener,noreferrer'
        );
      ...
      if (
        ...
        const seguro = await buscarSeguroPorId(seguroId, token, apiBaseUrl);
  ...
  const handleEnviar = async (e) => {
    ...
    setHistorico((prev) => [...prev, { role: 'user', text: textoMensagem }]);
    setMensagem('');
    setLoading(true);
